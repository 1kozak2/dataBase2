...
CREATE TABLE Pokój (ID int(6) NOT NULL AUTO_INCREMENT, Numer int(3) NOT NULL UNIQUE, Status varchar(24) NOT NULL, Description varchar(255), KlasaKlasaPokoju varchar(32) NOT NULL, PRIMARY KEY (ID));
...
CREATE TABLE Rezerwacje (ID int(6) NOT NULL UNIQUE, Termin_rezerwacji date NOT NULL, Termin_wymeldowania date NOT NULL, Wyzywienie varchar(15), DoZaplaty decimal(6, 2) NOT NULL, PokójID int(6) NOT NULL, `Klient zalogowanyID` int(6) NOT NULL, PaymentPayment_ID int(10) NOT NULL, PRIMARY KEY (ID, PokójID, `Klient zalogowanyID`, PaymentPayment_ID));
...
ALTER TABLE Pokój ADD CONSTRAINT `Pokoj nalezy do oznaczonej klasy` FOREIGN KEY (KlasaKlasaPokoju) REFERENCES Klasa (KlasaPokoju);
ALTER TABLE Pokój ADD CONSTRAINT `Pokoje do posprzatania sa przepisane do sprzataczki` FOREIGN KEY () REFERENCES Sprz?taczka ();
ALTER TABLE Rezerwacje ADD CONSTRAINT `Pokój jest wypo?yczony` FOREIGN KEY (PokójID) REFERENCES Pokój (ID);
